...
from loop_core.vendor.langextract.providers import builtin_registry
...
def load_plugins_once() -> None:
  ...
  try:
    ...
    if hasattr(eps, "select"):
      # Python 3.10+ API
      provider_eps = eps.select(group="loop_core.vendor.langextract.providers")
    elif hasattr(eps, "get"):
      # Python 3.9 API
      provider_eps = eps.get("loop_core.vendor.langextract.providers", [])
    else:
      # Fallback for older versions
      provider_eps = [
          ep
          for ep in eps
          if getattr(ep, "group", None) == "loop_core.vendor.langextract.providers"
      ]
...
def __getattr__(name: str):
  ...
  if name == "router":
    return importlib.import_module("loop_core.vendor.langextract.providers.router")
  elif name == "schemas":
    return importlib.import_module("loop_core.vendor.langextract.providers.schemas")
